...
site_update_schema = {
							"Login":[{
								...
								"table_obj": "Login",
								"fields":{
											"i_username":{"field_db":"username",   "key":True},
											"i_password":{"field_db":"password", "validation":{"required":True, "text_min_len":3, "text_max_len":20} }
								}
							}],
							"SiteEnv":[{ 	
								"module_name":"test_wc",
...
from guava.data_ui_model import DataUIModel
from guava.jinja_cust_funcs import JCFunc
...
@app.route('/test_form', methods=['GET', 'POST'], endpoint='test_form')
def test_form():
	breakpoint()
	data_ui = DataUIModel(  site_update_schema['Login'] , db.session, logger )
	
	if request.method=='POST':
		logger.debug('managing POST')
		return render_template('test_form.html', data_ui=data_ui, post_ok=True) #title='Sign In') #, form=form)
	else:
		return render_template('test_form.html', data_ui=data_ui) #title='Sign In') #, form=form)
...
if __name__ == '__main__':
	app.run(host="0.0.0.0", port=4701, debug=True)
